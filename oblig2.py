...
def analytisk(tid,t_m,v_m):
    a=((e*E2)/m_e)
     
    
    w = (e*np.linalg.norm(B))/m_e
    
    losnenX = 0.5*a[0]*tid**2
    losnenY = 0.5*a[1]*tid**2
    losnenZ = 0.5*a[2]*tid**2
    losntoX = r_m[:,0]*(1-np.cos(w*t_m))
    losntoY = -r_m[:,1]*(np.sin(w*t_m))
    losntoZ = t_m*(v_m[:,2])
    return  losnenX,losnenY,losnenZ ,losntoX,losntoY,losntoZ
...
if __name__ == "__main__":
    ...
    losnenX,losnenY,losnenZ ,losntoX,losntoY,losntoZ = analytisk(tid,t_m,v_m)
"""
    fig0 = plt.figure("Oppgave 1", figsize=(7,7)) 
    plt.plot(tid, R[:,0], "y", label="x(t)")
...
"""                        
    ...
    ax.plot(r_m[:,0],r_m[:,1],r_m[:,2])
    ax.plot(losntoX,losntoY,losntoZ)
    ax.set_xlabel("X[m]"), ax.set_ylabel("Y[m]"),ax.set_zlabel("Z[m]")
    p.title("Analytisk - $\delta t = 1fS \vec{V_{2}}$")
    p.show()
